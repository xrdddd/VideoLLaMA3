...
class ModelArguments:
    ...
    mm_attn_implementation: Optional[str] = field(default="sdpa")
...
class DataArguments:
    ...
    use_batch_flattening: bool = field(
        default=True,
        metadata={
            "help": "Whether to flatten the in-batch sequences of variable lengths."
        },
    )
...
class LazySupervisedDataset(Dataset):
        ...
        for conv in conversation:
            if conv["from"] == "human":
                ...
                chunks = conv["value"].split(
                    "<image>" if modal == "image" else "<video>"
                )
    ...
    def _convert_stream(self, data_dict):
        ...
        for message in conversation:
            ...
            while (
                frame_idx < len(timestamps) and timestamps[frame_idx] <= message["time"]
            ):
                messages.append(
                    {
                        "role": "stream",
...
def make_supervised_data_module(vlprocessor, data_args) -> Dict:
    ...
    train_dataset = LazySupervisedDataset(
        vlprocessor=vlprocessor, data_path=data_args.data_path, data_args=data_args
    )
...
class DataCollatorWithFlatteningForSupervisedDataset(object):
    ...
    def __call__(
        self, instances: Sequence[Dict], separator_id=-100
                ...
                torch.tensor(
                    list(
                        range(
                            len(
                                input_ids[idx][
                                    : self.vlprocessor.tokenizer.model_max_length
                                ]
                            )
                        )
                    )
...
def make_flattening_supervised_data_module(
    vlprocessor: transformers.ProcessorMixin, data_args
) -> Dict:
...
def train(attn_implementation=None):
    ...
    if model_args.vision_encoder is not None:
        ...
        model.config.image_aspect_ratio = (
            ...
            if "avt" not in model_args.vision_encoder
            else "avt"
        )
        ...
        model.config.image_token_length = data_args.image_token_length = (
            mm_projector.cal_proj_size(vision_encoder.num_patches_per_side)
        )
        # 3. check if alignment
        model.config.is_alignment = training_args.is_alignment = (
            data_args.is_alignment
        ) = (
            model.config.mm_projector_lr is not None
            and model.config.llm_lr is None
            and model.config.vision_encoder_lr is None
        )
    ...
    if local_rank == 0:
        print(type(model_args.vision_encoder))
        print("Current model:", model)
        print("Model config:", model.config)
        print("model.get_model():", model.get_model())
        print("its config:", model.get_model().config)
